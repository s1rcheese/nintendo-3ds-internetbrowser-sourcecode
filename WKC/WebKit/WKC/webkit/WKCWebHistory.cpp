...
class WKCWebHistoryWriter : public HistoryPropertyListWriter {
public:
    //WKCWebHistoryWriter(const WKCWebHistory::DateToEntriesMap&);

private:
    virtual void writeHistoryItems(BinaryPropertyListObjectStream&);

    //const WKCWebHistory::DateToEntriesMap& m_entriesByDate;
    Vector<WKCWebHistory::DateKey> m_dateKeys;
};

#if 0
WKCWebHistoryWriter::WKCWebHistoryWriter(const WKCWebHistory::DateToEntriesMap& entriesByDate)
...
void WKCWebHistoryWriter::writeHistoryItems(BinaryPropertyListObjectStream& stream)
{
#if 0
...
WKCWebHistory::WKCWebHistory()
//: m_refCount(0)
//, m_preferences(0)
{
#if 0
    gClassCount++;
    gClassNameCount.add("WKCWebHistory");
...
#if 0
    gClassCount--;
    gClassNameCount.remove("WKCWebHistory");
#endif
}

WKCWebHistory* WKCWebHistory::createInstance()
{
    WKCWebHistory* instance = new WKCWebHistory();
    instance->AddRef();
    return instance;
}

#if 0
long WKCWebHistory::postNotification(NotificationType notifyType, IPropertyBag* userInfo /*=0*/)
{
    IWebNotificationCenter* nc = WebNotificationCenter::defaultCenterInternal();
    long hr = nc->postNotificationName(getNotificationString(notifyType), static_cast<IWebHistory*>(this), userInfo);
    if (FAILED(hr))
        return hr;
...
BSTR WKCWebHistory::getNotificationString(NotificationType notifyType)
{
...
long WKCWebHistory::QueryInterface(REFIID riid, void** ppvObject)
{
    *ppvObject = 0;
...
unsigned long WKCWebHistory::AddRef(void)
{
    return ++m_refCount;
}

unsigned long WKCWebHistory::Release(void)
{
    unsigned long newRef = --m_refCount;
    if (!newRef)
        delete(this);
...
#if 0
static inline COMPtr<WKCWebHistory>& sharedHistoryStorage()
{
    DEFINE_STATIC_LOCAL(COMPtr<WKCWebHistory>, sharedHistory, ());
    return sharedHistory;
}

WKCWebHistory* WKCWebHistory::sharedHistory()
{
    return sharedHistoryStorage().get();
}

long WKCWebHistory::optionalSharedHistory( 
...
long WKCWebHistory::setOptionalSharedHistory( 
...
long WKCWebHistory::loadFromURL( 
    /* [in] */ BSTR url,
    /* [out] */ IWebError** error,
...
long WKCWebHistory::loadHistoryGutsFromURL(CFURLRef url, CFMutableArrayRef discardedItems, IWebError** /*error*/) //FIXME
{
...
long WKCWebHistory::saveHistoryGuts(CFURLRef url, IWebError** error)
{
    long hr = S_OK;
...
long WKCWebHistory::removeItems( 
    /* [in] */ int itemCount,
    /* [in] */ IWebHistoryItem** items)
{
    long hr;
...
long WKCWebHistory::removeAllItems( void)
{
...
long WKCWebHistory::orderedLastVisitedDays( 
    /* [out][in] */ int* count,
...
long WKCWebHistory::orderedItemsLastVisitedOnDay( 
    /* [out][in] */ int* count,
    /* [in] */ IWebHistoryItem** items,
...
long WKCWebHistory::data(IStream** stream)
{
    if (!stream)
        return E_POINTER;
...
long WKCWebHistory::setVisitedLinkTrackingEnabled(BOOL visitedLinkTrackingEnabled)
{
    PageGroup::setShouldTrackVisitedLinks(visitedLinkTrackingEnabled);
    return S_OK;
}

long WKCWebHistory::removeAllVisitedLinks()
{
    PageGroup::removeAllVisitedLinks();
    return S_OK;
}

long WKCWebHistory::setHistoryItemLimit( 
    /* [in] */ int limit)
{
    if (!m_preferences)
        return E_FAIL;
...
long WKCWebHistory::historyItemLimit( 
...
long WKCWebHistory::setHistoryAgeInDaysLimit( 
    /* [in] */ int limit)
{
    if (!m_preferences)
        return E_FAIL;
...
long WKCWebHistory::historyAgeInDaysLimit( 
    /* [retval][out] */ int* limit)
{
    if (!m_preferences)
        return E_FAIL;
...
long WKCWebHistory::removeItem(IWebHistoryItem* entry)
{
    long hr = S_OK;
    BSTR urlBStr = 0;
...
long WKCWebHistory::addItem(IWebHistoryItem* entry, bool discardDuplicate, bool* added)
{
    long hr = S_OK;
...
void WKCWebHistory::visitedURL(const KURL& url, const String& title, const String& httpMethod, bool wasFailure, bool increaseVisitCount)
{
...
long WKCWebHistory::itemForURLString(
...
long WKCWebHistory::removeItemForURLString(CFStringRef urlString)
{
    IWebHistoryItem* entry = (IWebHistoryItem*) CFDictionaryGetValue(m_entriesByURL.get(), urlString);
    if (!entry) 
        return E_FAIL;

    long hr = removeItemFromDateCaches(entry);
...
COMPtr<IWebHistoryItem> WKCWebHistory::itemForURLString(const String& urlString) const
{
...
long WKCWebHistory::addItemToDateCaches(IWebHistoryItem* entry)
{
    long hr = S_OK;
...
long WKCWebHistory::removeItemFromDateCaches(IWebHistoryItem* entry)
{
    long hr = S_OK;
...
static inline WKCWebHistory::DateKey dateKey(CFAbsoluteTime date)
{
...
bool WKCWebHistory::findKey(DateKey* key, CFAbsoluteTime forDay)
{
    ASSERT_ARG(key, key);
...
long WKCWebHistory::insertItem(IWebHistoryItem* entry, DateKey dateKey)
{
...
CFAbsoluteTime WKCWebHistory::timeToDate(CFAbsoluteTime time)
{
...
long WKCWebHistory::ageLimitDate(CFAbsoluteTime* time)
{
    ...
    long hr = historyAgeInDaysLimit(&historyLimitDays);
    if (FAILED(hr))
        return hr;
...
void WKCWebHistory::addVisitedLinksToPageGroup(PageGroup& group)
{
    CFDictionaryApplyFunction(m_entriesByURL.get(), addVisitedLinkToPageGroup, &group);
}

RetainPtr<CFDataRef> WKCWebHistory::data() const
{
