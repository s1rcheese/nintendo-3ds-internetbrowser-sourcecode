...
static LanguageSet* gLanguages = 0;
#endif
static bool isSupportedJavaScriptLanguage(const String& language)
{
#if PLATFORM(WKC)
    if (!gLanguages) {
        gLanguages = new LanguageSet();
    }
    LanguageSet& languages = *gLanguages;
#else
    typedef HashSet<String, CaseFoldingHash> LanguageSet;
...
#if PLATFORM(WKC)
void ScriptElement_deleteSharedInstance()
{
    delete gLanguages;
}

void ScriptElement_resetVariables()
{
    gLanguages = 0;
}
#endif
