...
#if 1// Patch of https://bugs.webkit.org/show_bug.cgi?id=30508
...
XMLTokenizer::~XMLTokenizer()
{
    ...
    // The XMLTokenizer will always be detached before being destroyed.
...
#if PLATFORM(WKC)
        // JavaScript may cause the tokenizer to detach during xmlParseChunk
...
void XMLTokenizer::endElementNs()
{
    ...
    {
#if PLATFORM(WKC)
        // JavaScript can detach the tokenizer.  Make sure this is not released
...
#if 1
    // added at webkit.org trunk r56420
    if (m_parserStopped)
        return;
#endif
...
#if PLATFORM(WKC)
void XMLTokenizer_resetVariables()
{
    didInit = false;
    globalDescriptor = 0;
    libxmlLoaderThread = 0;
}
#endif
