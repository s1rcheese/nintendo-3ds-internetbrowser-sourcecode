...
void Document::setDocType(PassRefPtr<DocumentType> docType)
{
    ...
    if (m_docType && !ownerElement()
        ...
        if (FrameView* frameView = view())
            frameView->frame()->loader()->updateViewport();
    }
#endif
...
#if PLATFORM(WKC)
static AtomicString* gXmlnsNamespaceURI = 0;
static AtomicString* gXmlns = 0;
static AtomicString* gXml = 0;
#endif

bool Document::hasPrefixNamespaceMismatch(const QualifiedName& qName)
{
#if PLATFORM(WKC)
    if (!gXmlnsNamespaceURI) {
        gXmlnsNamespaceURI = new AtomicString("http://www.w3.org/2000/xmlns/");
    }
    const AtomicString& xmlnsNamespaceURI = *gXmlnsNamespaceURI;
    if (!gXmlns) {
        gXmlns = new AtomicString("xmlns");
    }
    const AtomicString& xmlns = *gXmlns;
    if (!gXml) {
        gXml = new AtomicString("xml");
    }
    const AtomicString& xml = *gXml;
#else
...
void Document::processMetadataSettings(const String& content)
{
    ...
    // added nil-check as https://bugs.webkit.org/show_bug.cgi?id=36312
    // 101223 ACCESS Co.,Ltd.
    if (!this->frame())
        return;
    if (!this->frame()->settings())
        return;
...
#if 1
        // added at webkit.org trunk r57012
...
#if PLATFORM(WKC)
void Document::deleteSharedInstance()
{
    delete gXmlnsNamespaceURI;
    delete gXmlns;
    delete gXml;
    delete documentsThatNeedStyleRecalc;
}

void Document::resetVariables()
{
    gXmlnsNamespaceURI = 0;
    gXmlns = 0;
    gXml = 0;

    documentsThatNeedStyleRecalc = 0;
    gDocID = 0;
}
#endif
