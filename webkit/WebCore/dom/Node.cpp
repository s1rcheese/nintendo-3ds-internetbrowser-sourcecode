...
#ifndef NDEBUG
#if PLATFORM(WKC)
static WTF::RefCountedLeakCounter* gNodeCounter = 0;
#else
...
#if PLATFORM(WKC)
static HashSet<Node*>* gIgnoreSet = 0;
#else
...
#endif
{
    ...
    if (shouldIgnoreLeaks) {
        if (!gIgnoreSet) {
            gIgnoreSet = new HashSet<Node*>();
        }
        HashSet<Node*>& ignoreSet = *gIgnoreSet;
        ignoreSet.add(this);
    } else {
        if (!gNodeCounter) {
            gNodeCounter = new WTF::RefCountedLeakCounter("WebCoreNode");
        }
        WTF::RefCountedLeakCounter& nodeCounter = *gNodeCounter;
...
#if 1
    // modified at webkit.org trunk r53501
    if (!isAttributeNode())
...
#if PLATFORM(WKC)
static NodeRareData::NodeRareDataMap* gDataMap = 0;
NodeRareData::NodeRareDataMap& NodeRareData::rareDataMap()
{
    if (!gDataMap) {
        gDataMap = new NodeRareDataMap;
    }
    return *gDataMap;
}

void NodeRareData::deleteSharedInstance()
{
    delete gDataMap;
}
void NodeRareData::resetVariables()
{
    gDataMap = 0;
}

void Node::deleteSharedInstance()
{
...
{
    gNodesDispatchingSimulatedClicks = 0;
#ifndef NDEBUG
    willMoveToNewOwnerDocumentWasCalled = false;
    didMoveToNewOwnerDocumentWasCalled = false;
    shouldIgnoreLeaks = false;
    gNodeCounter = 0;
    gIgnoreSet = 0;
#endif
