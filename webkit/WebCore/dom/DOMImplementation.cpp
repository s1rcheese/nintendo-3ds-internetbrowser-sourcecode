...
#if PLATFORM(WKC)
static RegularExpression* gXmlTypeRegExp = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gXmlTypeRegExp) {
        gXmlTypeRegExp = new RegularExpression(String("^") + validChars + "+/" + validChars + "+\\+xml$", TextCaseSensitive);
    }
    RegularExpression& xmlTypeRegExp = *gXmlTypeRegExp;
#else
...
#if PLATFORM(WKC)
void DOMImplementation::deleteSharedInstance()
{
    delete gXmlTypeRegExp;
}

void DOMImplementation::resetVariables()
{
    gXmlTypeRegExp = 0;
}
#endif
