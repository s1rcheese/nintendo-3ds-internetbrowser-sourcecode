...
#if PLATFORM(WKC)
static AtomicString* gSubmit = 0;
static AtomicString* gButton = 0;
static AtomicString* gReset = 0;
#endif
...
#if PLATFORM(WKC)
            if (!gSubmit) {
                gSubmit = new AtomicString("submit");
            }
            return *gSubmit;
#else
...
#if PLATFORM(WKC)
            if (!gButton) {
                gButton = new AtomicString("button");
            }
            return *gButton;
#else
...
#if PLATFORM(WKC)
void HTMLButtonElement::deleteSharedInstance()
{
    delete gSubmit;
    delete gButton;
    delete gReset;
...
void HTMLButtonElement::resetVariables()
{
    gSubmit = 0;
    gButton = 0;
    gReset = 0;
