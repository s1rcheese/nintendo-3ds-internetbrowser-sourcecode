...
#if PLATFORM(WKC)
static AtomicString* gSelectMultiple = 0;
static AtomicString* gSelectOne = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gSelectMultiple) {
        gSelectMultiple = new AtomicString("select-multiple");
    }
    const AtomicString& selectMultiple = *gSelectMultiple;
    if (!gSelectOne) {
        gSelectOne = new AtomicString("select-one");
    }
    const AtomicString& selectOne = *gSelectOne;
#else
...
#if PLATFORM(WKC)
void HTMLSelectElement::deleteSharedInstance()
{
    delete gSelectMultiple;
    delete gSelectOne;
}

void HTMLSelectElement::resetVariables()
{
    gSelectMultiple = 0;
    gSelectOne = 0;
}
