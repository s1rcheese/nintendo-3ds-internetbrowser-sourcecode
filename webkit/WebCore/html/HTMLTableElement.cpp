...
#if PLATFORM(WKC)
static AtomicString* cellBorderNames[] = { 0, 0, 0, 0, 0 };
#endif
...
#if PLATFORM(WKC)
    if (!cellBorderNames[0]) {
        cellBorderNames[0] = new AtomicString("none");
        cellBorderNames[1] = new AtomicString("solid");
        cellBorderNames[2] = new AtomicString("inset");
        cellBorderNames[3] = new AtomicString("solid-cols");
        cellBorderNames[4] = new AtomicString("solid-rows");
    }
#else
...
#if PLATFORM(WKC)
void HTMLTableElement::deleteSharedInstance()
{
    int size = sizeof(cellBorderNames) / sizeof(cellBorderNames[0]);

    for (int i = 0; i < size; i++) {
        delete cellBorderNames[i];
        cellBorderNames[i] = 0;
    }
}

void HTMLTableElement::resetVariables()
{
    for (int i=0; i<(sizeof(cellBorderNames)/sizeof(cellBorderNames[0])); i++) {
        cellBorderNames[i] = 0;
    }
}
#endif
