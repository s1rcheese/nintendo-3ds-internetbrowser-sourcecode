...
#if 1
    // modified at webkit.org trunk r55203
...
#if PLATFORM(WKC)
static FunctionMap* ggFunctionMap = 0;
#endif
...
#if PLATFORM(WKC)
    if (!ggFunctionMap) {
        ggFunctionMap = new FunctionMap();
    }
    FunctionMap& gFunctionMap = *ggFunctionMap;
#else
...
#if PLATFORM(WKC)
static HashSet<AtomicStringImpl*>* gHeaderTags = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gHeaderTags) {
        gHeaderTags = new HashSet<AtomicStringImpl*>();
    }
    HashSet<AtomicStringImpl*>& headerTags = *gHeaderTags;
#else
...
#if PLATFORM(WKC)
static HashSet<AtomicStringImpl*>* gResidualStyleTags = 0;
#endif

bool HTMLParser::isResidualStyleTag(const AtomicString& tagName)
{
#if PLATFORM(WKC)
    if (!gResidualStyleTags) {
        gResidualStyleTags = new HashSet<AtomicStringImpl*>();
    }
    HashSet<AtomicStringImpl*>& residualStyleTags = *gResidualStyleTags;
#else
...
#if PLATFORM(WKC)
static HashSet<AtomicStringImpl*>* gUnaffectedTags = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gUnaffectedTags) {
        gUnaffectedTags = new HashSet<AtomicStringImpl*>();
    }
    HashSet<AtomicStringImpl*>& unaffectedTags = *gUnaffectedTags;
#else
...
#if 1
    // added at webkit.org trunk r55203
    if (m_isParsingFragment)
        return;
#endif
...
#if PLATFORM(WKC)
void HTMLParser::deleteSharedInstance()
{
    delete ggFunctionMap;
    delete gHeaderTags;
    delete gResidualStyleTags;
    delete gUnaffectedTags;
}

void HTMLParser::resetVariables()
{
    ggFunctionMap = 0;
    gHeaderTags = 0;
