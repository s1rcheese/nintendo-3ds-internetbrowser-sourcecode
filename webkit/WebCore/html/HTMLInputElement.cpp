...
#if PLATFORM(WKC)
static InputTypeMap* gTypeMap = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gTypeMap) {
        gTypeMap = (InputTypeMap *)createTypeMap();
    }
    const InputTypeMap* typeMap = gTypeMap;
#else
...
#if PLATFORM(WKC)
static AtomicString* gFormControlTypes = 0;
const AtomicString& HTMLInputElement::formControlType() const
{
    if (!gFormControlTypes) {
        gFormControlTypes = (AtomicString *)createFormControlTypes();
    }
    return gFormControlTypes[inputType()];
}
#else
const AtomicString& HTMLInputElement::formControlType() const
{
...
{
#if 0
    // removed at webkit.org trunk r57012
    if (!autoComplete())
        return false;
#endif
    switch (inputType()) {
        ...
        case URL:
#if 1
        // added at webkit.org trunk r57013
        case WEEK: {
...
#if PLATFORM(WKC)
void HTMLInputElement::deleteSharedInstance()
{
    delete gTypeMap;
    delete[] gFormControlTypes;
}

void HTMLInputElement::resetVariables()
{
    gTypeMap = 0;
    gFormControlTypes = 0;
}
