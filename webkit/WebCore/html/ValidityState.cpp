...
#if PLATFORM(WKC)
static AtomicString* gEmailPattern = 0;
static RegularExpression* gRegExp = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gEmailPattern) {
        gEmailPattern = new AtomicString(EMAIL_PATTERN);
    }
    AtomicString& emailPattern = *gEmailPattern;
    if (!gRegExp) {
        gRegExp = new RegularExpression(emailPattern, TextCaseInsensitive);
    }
    RegularExpression& regExp = *gRegExp;
...
#if PLATFORM(WKC)
void ValidityState::deleteSharedInstance()
{
    delete gEmailPattern;
    delete gRegExp;
}

void ValidityState::resetVariables()
{
    gEmailPattern = 0;
    gRegExp = 0;
}
