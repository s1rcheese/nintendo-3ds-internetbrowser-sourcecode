...
#if PLATFORM(WKC)
static AtomicString* gOptgroup = 0;
const AtomicString& HTMLOptGroupElement::formControlType() const
{
    if (!gOptgroup) {
        gOptgroup = new AtomicString("optgroup");
    }
    return *gOptgroup;
}
#else
...
#if PLATFORM(WKC)
void HTMLOptGroupElement::deleteSharedInstance()
{
    delete gOptgroup;
}

void HTMLOptGroupElement::resetVariables()
{
    gOptgroup = 0;
}
#endif
