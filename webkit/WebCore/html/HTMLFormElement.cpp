...
#if PLATFORM(WKC)
static int64_t gNextIdentifier = 0;
static int64_t generateFormDataIdentifier()
{
    // Initialize to the current time to reduce the likelihood of generating
    // identifiers that overlap with those from past/future browser sessions.
    if (gNextIdentifier==0) {
        gNextIdentifier = static_cast<int64_t>(currentTime() * 1000000.0);
    }
    return ++gNextIdentifier;
}
#else
static int64_t generateFormDataIdentifier()
{
...
{
#if 1
    // added at webkit.org trunk r54242
...
#if PLATFORM(WKC)
void HTMLFormElement::resetVariables()
{
    gNextIdentifier = 0;
}
#endif
