...
PassRefPtr<DocumentFragment> HTMLElement::createContextualFragment(const String &html)
{
    ...
    return Element::createContextualFragment(html);
#else
...
#if 1
        // modified at webkit.org trunk r75519
...
#if PLATFORM(WKC)
static HashSet<AtomicStringImpl*>* gTagList = 0;
#endif
bool HTMLElement::isRecognizedTagName(const QualifiedName& tagName)
{
#if PLATFORM(WKC)
    if (!gTagList) {
        gTagList = new HashSet<AtomicStringImpl*>();
    }
    HashSet<AtomicStringImpl*>& tagList = *gTagList;
#else
...
#if PLATFORM(WKC)
static HashSet<AtomicStringImpl*>* gInlineTagList = 0;
#endif
static HashSet<AtomicStringImpl*>* inlineTagList()
{
#if PLATFORM(WKC)
    if (!gInlineTagList) {
        gInlineTagList = new HashSet<AtomicStringImpl*>();
    }
    HashSet<AtomicStringImpl*>& tagList = *gInlineTagList;
#else
    DEFINE_STATIC_LOCAL(HashSet<AtomicStringImpl*>, tagList, ());
#endif
    if (tagList.isEmpty()) {
...
#if PLATFORM(WKC)
static HashSet<AtomicStringImpl*>* gBlockTagList = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gBlockTagList) {
        gBlockTagList = new HashSet<AtomicStringImpl*>();
    }
    HashSet<AtomicStringImpl*>& tagList = *gBlockTagList;
#else
    DEFINE_STATIC_LOCAL(HashSet<AtomicStringImpl*>, tagList, ());
#endif
    if (tagList.isEmpty()) {
...
#if PLATFORM(WKC)
void HTMLElement::deleteSharedInstance()
{
    delete gTagList;
    delete gInlineTagList;
    delete gBlockTagList;
}

void HTMLElement::resetVariables()
{
    gTagList = 0;
    gInlineTagList = 0;
    gBlockTagList = 0;
