...
#if PLATFORM(WKC)
static AtomicString* gTextArea = 0;
const AtomicString& HTMLTextAreaElement::formControlType() const
{
    if (!gTextArea) {
        gTextArea = new AtomicString("textarea");
    }
    return *gTextArea;
}
#else
...
#if PLATFORM(WKC)
void HTMLTextAreaElement::deleteSharedInstance()
{
    delete gTextArea;
}

void HTMLTextAreaElement::resetVariables()
{
    gTextArea = 0;
}
#endif
