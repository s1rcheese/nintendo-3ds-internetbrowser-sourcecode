...
void RenderFrameSet::calcPrefWidths()
{
    ...
    if (!parent()->isFrameSet() && !frameSet()->document()->frame()->ownerElement())
...
void RenderFrameSet::positionFramesWithFlattening()
{
    ...
    for(int r = 0; r < rows && !out; r++) {
        for(int c = 0; c < cols; c++) {
            ...
            // row size set the highest one of the column.
            if (c==0 || m_rows.m_sizes[r] < child->height()) {
...
bool RenderFrameSet::flattenFrameset() const
{
    ...
    return frameSet()->document()->frame() && frameSet()->document()->frame()->settings()->flatFrameSetLayoutEnabled();
}
#endif
