...
void BidiRun::destroy()
{
    ...
    renderArena->renderarena_free(*reinterpret_cast<size_t*>(this), this);
#else
...
#if PLATFORM(WKC)
    if (!bidiRunCounter) {
        bidiRunCounter = new RefCountedLeakCounter("BidiRun");
    }
...
{
#if PLATFORM(WKC)
    // This will give an application a chance to break heavy loop. 0 is dummy.
    wkcMemoryCheckAvailabilityPeer(0);
#endif
...
#if PLATFORM(WKC)
static AtomicString* ellipsisStr = 0;
#endif
...
#if PLATFORM(WKC)
    if (!ellipsisStr) {
        ellipsisStr = new AtomicString(&horizontalEllipsis, 1);
    }
#else
...
#if PLATFORM(WKC)
void RenderBlock::deleteSharedInstanceLineLayout()
{
    delete ellipsisStr;

#ifndef NDEBUG
    delete bidiRunCounter;
#endif
}

void RenderBlock::resetVariablesLineLayout()
{
    ellipsisStr = 0;
    numLines = 8;
