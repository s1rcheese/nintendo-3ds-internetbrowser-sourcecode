...
#if PLATFORM(WKC)
static RenderStyle* s_defaultStyle = 0;
inline RenderStyle* defaultStyle()
{
    if (!s_defaultStyle) {
        s_defaultStyle = RenderStyle::createDefaultStyle().releaseRef();
    }
...
#if PLATFORM(WKC)
static TransformOperations* gOps = 0;

const TransformOperations& RenderStyle::initialTransform()
{
    if (!gOps) {
        gOps = new TransformOperations();
    }
    return *gOps;
}

void RenderStyle::deleteSharedInstance()
{
//    if (s_defaultStyle)
//        s_defaultStyle->deref();
    delete gOps;
}

void RenderStyle::resetVariables()
{
    gOps = 0;
    s_defaultStyle = 0;
}
