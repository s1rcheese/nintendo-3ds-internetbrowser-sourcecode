...
#if 1
                                    // added at webkit.org trunk r63772 (fix is modified for this code)
...
#if 0
        // removed at webkit.org trunk r53525
...
#if 1
            // modified at webkit.org trunk r63048
...
#if PLATFORM(WKC)
static ContinuationOutlineTableMap* gTable = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gTable) {
        gTable = new ContinuationOutlineTableMap();
    }
    return gTable;
#else
...
#if 1
    // added at webkit.org trunk r61044
...
#if 1
// added at webkit.org trunk r67281
...
void RenderBlock::updateFirstLetter()
{
...
#if 1
    // modified at webkit.org trunk r56297
    ...
    if (!currChild)
        return;
#if 0
    // removed at webkit.org trunk r67281
    RenderObject* firstLetterContainer = currChild->parent();
#endif
    // If the child already has style, then it has already been created, so we just want
    // to update it.
#if 1
    // modified at webkit.org trunk r67281
...
#if PLATFORM(WKC)
void RenderBlock::deleteSharedInstance()
{
    delete gTable;
    delete gColumnInfoMap;
    delete gPercentHeightDescendantsMap;
    delete gPercentHeightContainerMap;
    delete gDelayedUpdateScrollInfoSet;

    deleteSharedInstanceLineLayout();
}

void RenderBlock::resetVariables()
{
    gTable = 0;
    gColumnInfoMap = 0;
    gPercentHeightDescendantsMap = 0;
    gPercentHeightContainerMap = 0;
    gDelayUpdateScrollInfo = 0;
    gDelayedUpdateScrollInfoSet = 0;
    
    resetVariablesLineLayout();
}
#endif
