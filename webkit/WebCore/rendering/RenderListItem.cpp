...
#if PLATFORM(WKC)
static String* gStaticNullString = 0;
const String& RenderListItem::markerText() const
{
    if (m_marker)
        return m_marker->text();
    if (!gStaticNullString) {
        gStaticNullString = new String();
    }
    return *gStaticNullString;
}

void RenderListItem::deleteSharedInstance()
{
    delete gStaticNullString;
}

void RenderListItem::resetVariables()
{
    gStaticNullString = 0;
}
