...
#ifndef NDEBUG
#if PLATFORM(WKC)
static WTF::RefCountedLeakCounter* gPageCounter = 0;
#else
...
#if PLATFORM(WKC)
    if (!gPageCounter) {
        gPageCounter = new WTF::RefCountedLeakCounter("Page");
    }
    WTF::RefCountedLeakCounter& pageCounter = *gPageCounter;
#endif
...
#if PLATFORM(WKC)
    if (!gNullString) {
        gNullString = new String();
    }
    String& nullString = *gNullString;
#else
...
#if PLATFORM(WKC)
void Page::deleteSharedInstance()
{
    delete gNullString;
    delete allPages;
#ifndef NDEBUG
