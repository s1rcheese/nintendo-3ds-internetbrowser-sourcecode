...
static ShorthandMap* gShorthandMap = 0;

CSSPropertyLonghand longhandForProperty(int propertyID)
{
    CSSPropertyLonghand_createSharedInstance();

    if (gShorthandMap->isEmpty())
        initShorthandMap(*gShorthandMap);

    return gShorthandMap->get(propertyID);
}

bool CSSPropertyLonghand_createSharedInstance()
{
    if (!gShorthandMap) {
        gShorthandMap = new ShorthandMap();
        if (!gShorthandMap)
            return false;
    }
...
void CSSPropertyLonghand_resetVariables()
{
    gShorthandMap = 0;
}

void CSSPropertyLonghand_deleteSharedInstance()
{
    delete gShorthandMap;
    gShorthandMap = 0;
}
