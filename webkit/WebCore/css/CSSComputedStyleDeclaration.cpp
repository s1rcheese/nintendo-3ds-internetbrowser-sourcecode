...
static const AtomicString* cursiveFamily = 0;
static const AtomicString* fantasyFamily = 0;
static const AtomicString* monospaceFamily = 0;
static const AtomicString* sansSerifFamily = 0;
static const AtomicString* serifFamily = 0;
static HashSet<int> *propertyIDSet = 0;

static bool CSSComputedStyleDeclaration_CreateSharedInstance()
{
    bool ret = false;

    if (!cursiveFamily) {
        cursiveFamily = new AtomicString("-webkit-cursive");
        if (!cursiveFamily)
            goto exit_func;
    }

    if (!fantasyFamily) {
        fantasyFamily = new AtomicString("-webkit-fantasy");
        if (!fantasyFamily)
            goto exit_func;
    }

    if (!monospaceFamily) {
        monospaceFamily = new AtomicString("-webkit-monospace");
        if (!monospaceFamily)
            goto exit_func;
    }

    if (!sansSerifFamily) {
        sansSerifFamily = new AtomicString("-webkit-sans-serif");
        if (!sansSerifFamily)
            goto exit_func;
    }

    if (!serifFamily) {
        serifFamily = new AtomicString("-webkit-serif");
        if (!serifFamily)
            goto exit_func;
    }
...
exit_func:
    if (!ret)
        CSSComputedStyleDeclaration_deleteSharedInstance();

    return ret;
}

void CSSComputedStyleDeclaration_resetVariables()
{
    cursiveFamily = 0;
...
void CSSComputedStyleDeclaration_deleteSharedInstance()
{
    delete cursiveFamily;
    cursiveFamily = 0;

    delete fantasyFamily;
    fantasyFamily = 0;

    delete monospaceFamily;
    monospaceFamily = 0;

    delete sansSerifFamily;
    sansSerifFamily = 0;

    delete serifFamily;
    serifFamily = 0;

    delete propertyIDSet;
    propertyIDSet = 0;
}

static int identifierForFamily(const AtomicString& family)
{
    if (!CSSComputedStyleDeclaration_CreateSharedInstance())
        return 0;
