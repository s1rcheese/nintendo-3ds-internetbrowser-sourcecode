...
static PseudoState pseudoState = PseudoUnknown;
...
#if PLATFORM(WKC)
static MediaQueryEvaluator* gStaticScreenEval = 0;
static MediaQueryEvaluator* gStaticPrintEval = 0;
static const MediaQueryEvaluator& screenEval()
{
    if (!gStaticScreenEval) {
        gStaticScreenEval = new MediaQueryEvaluator("screen");
    }
    return *gStaticScreenEval;
}

static const MediaQueryEvaluator& printEval()
{
    if (!gStaticPrintEval) {
        gStaticPrintEval = new MediaQueryEvaluator("print");
    }
    return *gStaticPrintEval;
}
#else
static const MediaQueryEvaluator& screenEval()
{
...
static CSSStyleSheet* gParsers[EParserTypes] = {0};
static CSSStyleSheet* parseUASheet(int type, const String& str)
{
...
static CSSStyleSheet* parseUASheet(int type, const char* characters, unsigned size)
{
    return parseUASheet(type, String(characters, size));
}
#else
...
#if PLATFORM(WKC)
    CSSStyleSheet* defaultSheet = parseUASheet(EFull, defaultRules);
#else
...
#if PLATFORM(WKC)
    CSSStyleSheet* quirksSheet = parseUASheet(EFullQuirks, quirksRules);
#else
...
#if PLATFORM(WKC)
    simpleDefaultStyleSheet = parseUASheet(ESimpleUA, simpleUserAgentStyleSheet, strlen(simpleUserAgentStyleSheet));
#else
...
#if PLATFORM(WKC)
    defaultViewSourceStyle->addRulesFromSheet(parseUASheet(EViewSource, sourceUserAgentStyleSheet, sizeof(sourceUserAgentStyleSheet)), screenEval());
#else
...
#if PLATFORM(WKC)
        CSSStyleSheet* svgSheet = parseUASheet(ESVG, svgUserAgentStyleSheet, sizeof(svgUserAgentStyleSheet));
#else
...
#if PLATFORM(WKC)
        CSSStyleSheet* mathMLSheet = parseUASheet(EMathML, mathmlUserAgentStyleSheet, sizeof(mathmlUserAgentStyleSheet));
#else
...
#if PLATFORM(WKC)
        CSSStyleSheet* wmlSheet = parseUASheet(EWML, wmlUserAgentStyleSheet, sizeof(wmlUserAgentStyleSheet));
#else
...
#if PLATFORM(WKC)
        CSSStyleSheet* mediaControlsSheet = parseUASheet(EMedia, mediaRules);
#else
...
PassRefPtr<RenderStyle> CSSStyleSelector::pseudoStyleForElement(PseudoId pseudo, Element* e, RenderStyle* parentStyle)
{
...
#if 1
    // modified at webkit.org trunk r67255
    m_style = RenderStyle::create();
    if (parentStyle)
    ...
    if (m_matchedDecls.isEmpty())
        return 0;
#if 0
    // removed at webkit.org trunk r67255
    m_style = RenderStyle::create();
    if (parentStyle)
        m_style->inheritFrom(parentStyle);
#endif
...
/* static */ void CSSStyleSelector::resetVariables()
{
    s_styleNotYetAvailable = 0;
}

/* static */ void CSSStyleSelector::deleteSharedInstance()
{
...
static HashSet<AtomicStringImpl*>* gHtmlCaseInsensitiveAttributesSet = 0;
static bool gHtmlCaseInsensitiveAttributesSetInit = false;

static bool htmlAttributeHasCaseInsensitiveValue(const QualifiedName& attr)
{
    if (!gHtmlCaseInsensitiveAttributesSetInit) {
        gHtmlCaseInsensitiveAttributesSet = createHtmlCaseInsensitiveAttributesSet();
        if (!gHtmlCaseInsensitiveAttributesSet)
            return false;

        gHtmlCaseInsensitiveAttributesSetInit = true;
    }

    bool isPossibleHTMLAttr = !attr.hasPrefix() && (attr.namespaceURI() == nullAtom);
    return isPossibleHTMLAttr && gHtmlCaseInsensitiveAttributesSet->contains(attr.localName().impl());
}

void CSSStyleSelector_resetVariables()
{
    CSSStyleSelector::resetVariables();

    gHtmlCaseInsensitiveAttributesSet = 0;
    gHtmlCaseInsensitiveAttributesSetInit = false;
    ...
    for (int i=0; i<EParserTypes; i++) {
        gParsers[i] = 0;
    }
}

void CSSStyleSelector_deleteSharedInstance()
{
    CSSStyleSelector::deleteSharedInstance();

    if (gHtmlCaseInsensitiveAttributesSetInit) {
        delete gHtmlCaseInsensitiveAttributesSet;
    }
    gHtmlCaseInsensitiveAttributesSet = 0;
    gHtmlCaseInsensitiveAttributesSetInit = false;
    ...
    delete gStaticScreenEval;
    gStaticScreenEval = 0;

    delete gStaticPrintEval;
    gStaticPrintEval = 0;
