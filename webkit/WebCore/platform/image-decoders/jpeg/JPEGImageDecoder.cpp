...
#if PLATFORM(WKC) && ENABLE(WKC_HIDE_SETJMP)
    if (!m_reader && !m_failed) {
        try {
            m_reader = new JPEGImageReader(this);
        } catch (SETJMPERR& /*e*/) {
            wkcMemoryNotifyNoMemoryPeer(0xffffffff, false);
        } catch (std::bad_alloc&) {
            throw std::bad_alloc();
        }
    }
...
#if PLATFORM(WKC) && ENABLE(IMAGE_DECODER_DOWN_SAMPLING)
        double scalex = (double)bufferWidth / (double)size().width();
        double scaley = (double)bufferHeight / (double)size().height();
        if (!buffer.setSize(bufferWidth, bufferHeight, scalex, scaley, m_reduceDepth)) {
#else
