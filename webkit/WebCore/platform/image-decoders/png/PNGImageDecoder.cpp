...
public:
...
#if PLATFORM(WKC)
        WTF::fastFree(m_interlaceBuffer);
#else
    ...
    void createInterlaceBuffer(int size) {
        ...
        if (!rv.getValue(m_interlaceBuffer)) {
            wkcMemoryNotifyMemoryAllocationErrorPeer(size, WKC_MEMORYALLOC_TYPE_IMAGE);
#if ENABLE(WKC_HIDE_SETJMP)
            throw SETJMPERR();
#else
...
#endif
#if PLATFORM(WKC) && ENABLE(IMAGE_DECODER_DOWN_SAMPLING)
        double scalex = (double)width / (double)size().width();
        double scaley = (double)height / (double)size().height();
        if (!buffer.setSize(width, height, scalex, scaley, m_reduceDepth)) {
#else
