...
#ifndef NDEBUG
#if PLATFORM(WKC)
static RefCountedLeakCounter* gCcachedResourceLeakCounter = 0;
#else
...
#ifndef NDEBUG
#if PLATFORM(WKC)
    if (!gCcachedResourceLeakCounter) {
        gCcachedResourceLeakCounter = new RefCountedLeakCounter("CachedResource");
    }
    RefCountedLeakCounter& cachedResourceLeakCounter = *gCcachedResourceLeakCounter;
#endif
...
#if PLATFORM(WKC)
static AtomicString* gContentHeaderPrefix = 0;
#endif
...
#if PLATFORM(WKC)
    if (!gContentHeaderPrefix) {
        gContentHeaderPrefix = new AtomicString("content-");
    }
    const AtomicString& contentHeaderPrefix = *gContentHeaderPrefix;
#else
...
static AtomicString* gETagHeader = 0;
#endif
...
bool CachedResource::canUseCacheValidator() const
{
    ...
    if (!gETagHeader) {
        gETagHeader = new AtomicString("etag");
    }
    const AtomicString& eTagHeader = *gETagHeader;
#else
...
#if PLATFORM(WKC)
void CachedResource::deleteSharedInstance()
{
    delete gContentHeaderPrefix;
    delete gLastModifiedHeader;
    delete gETagHeader;
#ifndef NDEBUG
    delete gCcachedResourceLeakCounter;
#endif
}

void CachedResource::resetVariables()
{
    gContentHeaderPrefix = 0;
    gLastModifiedHeader = 0;
    gETagHeader = 0;
#ifndef NDEBUG
    gCcachedResourceLeakCounter = 0;
#endif
}
#endif
