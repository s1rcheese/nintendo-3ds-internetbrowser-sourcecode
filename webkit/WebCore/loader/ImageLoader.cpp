...
#if PLATFORM(WKC)
static ImageEventSender* gSenderBeforeLoad = 0;
static ImageEventSender* gSenderLoad = 0;
static ImageEventSender& beforeLoadEventSender()
{
    if (!gSenderBeforeLoad) {
        gSenderBeforeLoad = new ImageEventSender(eventNames().beforeloadEvent);
    }
    return *gSenderBeforeLoad;
}

static ImageEventSender& loadEventSender()
{
    if (!gSenderLoad) {
        gSenderLoad = new ImageEventSender(eventNames().loadEvent);
    }
    return *gSenderLoad;
}
#else
...
#if PLATFORM(WKC)
void ImageLoader::deleteSharedInstance()
{
    delete gSenderBeforeLoad;
    delete gSenderLoad;
}

void ImageLoader::resetVariables()
{
    gSenderBeforeLoad = 0;
    gSenderLoad = 0;
}
