...
#if PLATFORM(WKC)
// If you want to disable heap backup block, comment out the macro below.
#define WKC_PREPARE_JS_HEAP_BACKUP_BLOCK 1
...
namespace JSC {
...
#if PLATFORM(WKC)
    const unsigned int kMaxBlockInfo = 350; // Initial Memory Size / BLOCK_SIZE
    ...
    struct CollectorHeapBlockInfo_ {
        size_t size;
    ...
    typedef struct CollectorHeapBlockInfo_ CollectorHeapBlockInfo;

    struct CollectorHeapStatistics_ {
        unsigned int numBlockInfo;
        CollectorHeapBlockInfo blockInfo[kMaxBlockInfo];
    };

    typedef struct CollectorHeapStatistics_ CollectorHeapStatistics;

    void initCollectorHeapStatistics(CollectorHeapStatistics&);
#endif
...
#if PLATFORM(WKC)
        void statistics(CollectorHeapStatistics&, int);
        static void setConfiguration(WKCMemoryJSConfig* config);

#if defined(WKC_PREPARE_JS_HEAP_BACKUP_BLOCK)
        bool isAllocationLimited() const { return !m_heap.backupBlock; }
#endif
#endif
...
#if PLATFORM(WINCE) || PLATFORM(SYMBIAN) || PLATFORM(WKC)
