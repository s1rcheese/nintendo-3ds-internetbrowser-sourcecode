...
#elif PLATFORM(WKC)
// The macro below enables steps dependent on sub-allocator.
// Comment out to disable them. Defining it to 0 is NOT enough.
#define WKC_ENABLE_SUBALLOCATOR_OPTIMISED_STEPS 1
#endif

#define COLLECT_ON_EVERY_ALLOCATION 0

#ifdef ENABLE_REPLACEMENT_FASTMALLOC
#undef malloc
#undef calloc
#undef realloc
#undef free

#define malloc WTF::fastMalloc
#define calloc WTF::fastCalloc
#define realloc WTF::fastRealloc
#define free WTF::fastFree
#endif /* ENABLE_REPLACEMENT_FASTMALLOC */
...
#if PLATFORM(WKC) && defined(WKC_PREPARE_JS_HEAP_BACKUP_BLOCK)
    ASSERT(!m_heap.backupBlock);
    allocateBlock(); // for backupBlock
    ASSERT(m_heap.backupBlock);
#endif
...
Heap::~Heap()
{
    ...
    // Remove this ASSERT in order to check memory leaks in WKCWebKitFinalize()
    // ASSERT(!m_globalData);
}
...
NEVER_INLINE CollectorBlock* Heap::allocateBlock()
{
    ...
    address = wkcMemoryZeroAllocAlignedXPeer(BLOCK_SIZE, BLOCK_SIZE, WKC_MEMORYRANGE_TYPE_JSHEAP);
    if (!address) {
        // (BLOCK_SIZE * 2 + uint_size) means (BLOCK_SIZE /* Alignment Size */ + (BLOCK_SIZE + uint_size) /* Allocation Size */)
        if (!tryFastZeroedMalloc(BLOCK_SIZE * 2 + uint_size).getValue(address)) {
            wkcMemoryNotifyNoMemoryPeer(BLOCK_SIZE * 2 + uint_size, true);
            if (!tryFastZeroedMalloc(BLOCK_SIZE * 2 + uint_size).getValue(address)) {
#if defined(WKC_PREPARE_JS_HEAP_BACKUP_BLOCK)
                wkcMemoryNotifyMemoryAllocationErrorPeer(BLOCK_SIZE, WKC_MEMORYALLOC_TYPE_JAVASCRIPT_HEAP);
                // Initialize block.
                CollectorBlock* block = m_heap.backupBlock;
                m_heap.backupBlock = 0;
                block->heap = this;
                clearMarkBits(block);
        ...
        unsigned int align = BLOCK_SIZE;
        unsigned int offset = (unsigned int)ptr & (align - 1);
        ...
        if (offset==0) {
            ...
            ptr += (- offset + align - uint_size);
            offset = align - offset;
        }

        memcpy(ptr, &offset, uint_size);
        ptr += uint_size;
...
#if PLATFORM(WKC) && defined(WKC_PREPARE_JS_HEAP_BACKUP_BLOCK)
    if (!m_heap.backupBlock) {
        m_heap.backupBlock = block;
    } else {
#endif
    // Initialize block.
...
NEVER_INLINE void Heap::freeBlock(size_t block)
{
    ...
    for ( ; it != end; ++it)
        (*it)->~JSCell();
#if PLATFORM(WKC) && defined(WKC_PREPARE_JS_HEAP_BACKUP_BLOCK)
    CollectorBlock* blockToBeFreed = m_heap.blocks[block];
    if (isAllocationLimited()) {
        // Reuse the unused block for a backup block.
        m_heap.backupBlock = blockToBeFreed;
        memset(m_heap.backupBlock, 0, BLOCK_SIZE);
#if defined(WKC_ENABLE_SUBALLOCATOR_OPTIMISED_STEPS)
    } else if (!wkcMemoryIsAlignedPeer((void*)(m_heap.backupBlock))
        && wkcMemoryIsAlignedPeer((void*)(blockToBeFreed))) {
        // BackupBlock is preferred to be a block from system than that from TCMalloc.
        freeBlockPtr(m_heap.backupBlock);
        m_heap.backupBlock = blockToBeFreed;
        memset(m_heap.backupBlock, 0, BLOCK_SIZE);
#endif
    } else {
        freeBlockPtr(blockToBeFreed);
    }
#else
...
NEVER_INLINE void Heap::freeBlockPtr(CollectorBlock* block)
{
    ...
    free(block);
#elif PLATFORM(WKC)
    if (!wkcMemoryFreeAlignedPeer(block) && block) {
      char *mem = (char*)block;
      unsigned int offset;
      unsigned int uint_size = (unsigned int)sizeof(unsigned int);

      memcpy(&offset, mem - uint_size, uint_size);
      mem -= offset;

      block = reinterpret_cast<CollectorBlock*>(mem);
...
#if PLATFORM(WKC) && defined(WKC_PREPARE_JS_HEAP_BACKUP_BLOCK)
    while (m_heap.usedBlocks < neededBlocks && !isAllocationLimited())
#else
...
#if PLATFORM(WKC) && defined(WKC_ENABLE_SUBALLOCATOR_OPTIMISED_STEPS)
    // Assumed that blocks allocated from TCMalloc are in the latter of m_heap.blocks array.
    // Freeing blocks from the end aims to free blocks from TCMalloc more than those from system.
    for (int j = m_heap.usedBlocks - 1; j >= 0 && m_heap.usedBlocks != neededBlocks; --j) {
        if (m_heap.blocks[j]->marked.isEmpty()) {
            freeBlock(j);
...
#elif PLATFORM(WKC)
    return wkcThreadGetStackBasePeer(wkcThreadCurrentThreadPeer());
#else
...
{
    unsigned int registers[64];
    memset(registers, 0, sizeof(registers));
    wkcThreadStoreRegistersPeer((void *)registers, sizeof(registers));
...
#if 1
// added at webkit.org trunk r73623
...
void Heap::addToStatistics(Heap::Statistics& statistics) const
{
    ...
    statistics.statistics_free += m_heap.usedBlocks * BLOCK_SIZE - (objectCount() * HeapConstants::cellSize);
#else
...
#if PLATFORM(WKC)
void Heap::setConfiguration(WKCMemoryJSConfig* config)
{
    ASSERT(config != NULL);

    config->fBlockSize = BLOCK_SIZE;
    config->fAlignment = BLOCK_SIZE;
}

static void setCollectorHeapStatistics(CollectorHeapStatistics& stat, int type, CollectorBlock* block, bool isBackupBlock)
{
    CollectorHeapBlockInfo* info;
    ...
    info->type = type;
    info->flag = ECollectorHeapBlockInfoFlagNone; // Ensure all flags are unset.

    if (wkcMemoryIsAlignedPeer((void*)block)) {
        info->flag |= ECollectorHeapBlockInfoFlagPeer;
        info->size = BLOCK_SIZE;
    } else {
        info->flag |= ECollectorHeapBlockInfoFlagTCMalloc;
        /*
            Actually the request memory size must be BLOCK_SIZE.
            However, the memory alignemt (BLOCK_SIZE) must be considered in JS Heap.
            Therefore, the request memory size must be BLOCK_SIZE x 2.
            "sizeof(unsigned int)" means that the memory area is needed to store "offset" from the head of address for memory alignement.
        */
        info->size = BLOCK_SIZE + BLOCK_SIZE + sizeof(unsigned int);
    }

    if (isBackupBlock) {
        info->flag |= ECollectorHeapBlockInfoFlagBackup;
    }

    size_t usedCellCount = block->marked.count();
    if (usedCellCount <= 1) {
        // If this block is in use, its last bit is always on, so the count is always more than 1.
        // What to know here is whether any cells are used for scripts, so ignore the last 1 bit.
        info->flag |= ECollectorHeapBlockInfoFlagNoCellUsed;
        info->usedCellCount = 0;
    } else {
        info->usedCellCount = usedCellCount - 1;
    }

    #define ASSERT_IF_THEN(test, then) ASSERT(!(test) || (then))
    // Ensure only one of the flags for memory manager is set.
    ASSERT_IF_THEN((info->flag & ECollectorHeapBlockInfoFlagPeer) == 0, (info->flag & ECollectorHeapBlockInfoFlagTCMalloc) != 0);
    ASSERT_IF_THEN((info->flag & ECollectorHeapBlockInfoFlagPeer) != 0, (info->flag & ECollectorHeapBlockInfoFlagTCMalloc) == 0);
    #undef ASSERT_IF_THEN

    ++stat.numBlockInfo;
}

void Heap::statistics(CollectorHeapStatistics& stat, int type)
{
    size_t numBlock = m_heap.usedBlocks;

    for (unsigned int i = 0; i < numBlock; i++) {
        if (m_heap.blocks[i]) {
            setCollectorHeapStatistics(stat, type, m_heap.blocks[i], false);
        }
    }
    if (m_heap.backupBlock) {
        setCollectorHeapStatistics(stat, type, m_heap.backupBlock, true);
    }
}

void initCollectorHeapStatistics(CollectorHeapStatistics& stat)
{
    CollectorHeapBlockInfo* info;
    stat.numBlockInfo = 0;

    for (unsigned int i = 0; i < kMaxBlockInfo; i++) {
        info = &stat.blockInfo[i];
        info->size = 0;
        info->type = ECollectorHeapTypeNone;
        info->flag = ECollectorHeapBlockInfoFlagNone;
        info->usedCellCount = 0;
    }
}
#endif /* PLATFORM(WKC) */
